...
<script>
import { getFoundUdisk, getLostUdisk } from "@/api/getList";
import { deletefoundUdisk, deletelostUdisk } from "@/api/deleteList";
  ...
  methods: {
    ...
    saveEdit: function() {
      if (this.form.id !== -1) {
        ...
        axios
          .post(
            process.env.BASE_API + "/udisk/" + this.message + "/edit",
            formData,
    ...
    deleteRow: function() {
      new Promise((resolve, reject) => {
        ...
        if (this.message === "found") {
          deletefoundUdisk(this.list[this.rowIndex].id)
            .then(response => {
          ...
          deletelostUdisk(this.list[this.rowIndex].id)
            .then(response => {
              console.log(response.data);
              Message.success("删除成功");
              this["Get" + this.message + "Udisk"]();
