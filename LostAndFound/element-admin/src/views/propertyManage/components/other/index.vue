  ...
  <div class="app-container">
    ...
    <div style="width: 1050px">
      <el-table :data="list" border>
        <el-table-column label="其他物件" align="center">
          <el-table-column center="true" prop="date" label="日期" width="100px" sortable/>
...
<script>
import { getFoundOther, getLostOther } from "@/api/getList";
import { deletefoundOther, deletelostOther } from "@/api/deleteList";
...
export default {
  ...
  mounted() {
    console.log("Get" + this.message + "Other");
    this["Get" + this.message + "Other"]();
  },
  watch: {
    currentRouter: {
      handler(newName, oldName) {
        if (newName === "other") {
          ...
          this.$store.dispatch("GetCurrentRouter", "");
          console.log("00000000000000000000000000");
        }
        console.log("111111111100000000000000000000000000");
        console.log(newName);
        console.log(oldName);
      },
      immediate: true
  ...
  methods: {
    ...
    saveEdit: function() {
      if (this.form.id !== -1) {
        ...
        axios
          .post(
            process.env.BASE_API + "/other/" + this.message + "/edit",
            formData,
    ...
    deleteRow: function() {
      new Promise((resolve, reject) => {
        ...
        if (this.message === "found") {
          deletefoundOther(this.list[this.rowIndex].id)
            .then(response => {
          ...
          deletelostOther(this.list[this.rowIndex].id)
            .then(response => {
              console.log(response.data);
              Message.success("删除成功");
              this["Get" + this.message + "Other"]();
